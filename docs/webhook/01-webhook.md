...
2. Can variables extracted in "Response Settings" have the same name as slots existing in the dialog flows?
   
Their name can be the same.  The slots will be filled with the extracted value. 

4. How does the extracted slot value change when the same webhook is called multiple times?
